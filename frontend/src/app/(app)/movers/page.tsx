...
/* Movers — a stock that moved big, and the Stalvian album that caught it. */

import { useInfiniteQuery, useMutation, useQueryClient } from "@tanstack/react-query";
import { toast } from "sonner";
import { fetchMovers, refreshFeeds } from "@/lib/api";
...
export default function MoversPage() {
  ...
  const refresh = useMutation({
    ...
    onSuccess: (result: { status: string; new_movers?: number }) => {
      ...
      if (result.status === "ok") {
        toast.success(
          result.new_movers ? `${result.new_movers} new stories pulled in` : "You're up to date"
        );
      } else {
    ...
    <div className="flex flex-col gap-12">
      <div className="flex flex-wrap items-end justify-between gap-6">
        <div className="flex flex-col gap-6">
          <Eyebrow icon="ph-chart-line-up">Movers</Eyebrow>
          <h1 className="display-md max-w-[720px] text-ink">
            The stock ripped.
            <br />
            Our albums caught it.
          </h1>
          <p className="max-w-[560px] text-[18px] leading-6 text-slate-500">
            Each story starts with a stock that moved a lot — then reveals the politician or
            hedge fund in Stalvian&apos;s albums that was already in the trade. Built for the
            &quot;they knew&quot; hook.
          </p>
        </div>
        <Button
      ...
      {data && items.length === 0 && (
        <EmptyState
          ...
          title="No mover stories yet"
          body="Mover stories are generated when a stock in one of the albums makes a big move. Pull the feed to check now."
          action={
